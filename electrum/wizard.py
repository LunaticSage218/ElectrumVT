...
# LARRY: THIS IS WHERE THE CHANGES WILL BE MADE FOR THE PAGES
...
class NewWalletWizard(KeystoreWizard):
    ...
    def __init__(self, daemon: 'Daemon', plugins: 'Plugins'):
        ...
        self.navmap = {
            ...
            'confirm_seed': {
                'next': lambda d: 'save_seed_virtual_token' if d.pop('save_seed_virtual_token', False)
                                  else ('confirm_ext' if self.wants_ext(d) else self.on_have_or_confirm_seed(d)),
            ...
            'save_seed_virtual_token': {
                'next': 'virtual_token_enroll', 
                'accept': lambda d: None, 
                'last': lambda d: False,
            },
            'virtual_token_enroll':{
                'next': 'virtual_token_encrypt_seed',
            },
            'virtual_token_encrypt_seed': {
                'next': lambda d: 'confirm_exit' if self.wants_ext(d) else self.on_have_or_confirm_seed(d),
                'accept': lambda d: self.maybe_master_pubkey(d) if not self.wants_ext(d) else None,
                'last': lambda d: self.is_single_password() and not self.is_multisig(d) and not self.wants_ext(d),
            },
            'retrieve_seed_virtual_token': {
                'last': True,
            },
            'confirm_ext': {
    ...
    def on_wallet_type(self, wizard_data: dict) -> str:
        ...
        return {
            ...
            'multisig': 'multisig',
            'imported': 'imported',
            'retrieve_vt_seed': 'retrieve_seed_virtual_token',
        }.get(t)
