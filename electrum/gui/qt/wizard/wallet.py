...
from PyQt6.QtWidgets import (QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QWidget,
                             QFileDialog, QSlider, QGridLayout, QDialog, QApplication, QPlainTextEdit, 
...
from electrum.gui.qt.wizard.virtual_token_utils.enrollment_worker import EnrollmentWorker  
...
class QENewWalletWizard(NewWalletWizard, QEAbstractWizard, MessageBoxMixin):
    ...
    def __init__(self, config: 'SimpleConfig', app: 'QElectrumApplication', plugins: 'Plugins', daemon: Daemon, path, *, start_viewstate=None):
        ...
        self.navmap_merge({
            ...
            'save_seed_virtual_token': {'gui': WCSaveSeedVirtualToken},
            'virtual_token_enroll': {'gui': WCVirtualTokenEnrollment},
            'virtual_token_encrypt_seed': {'gui': WCVirtualTokenEncryptSeed},
            'retrieve_seed_virtual_token': {'gui': WCRetrieveSeedVirtualToken},
...
class WCWalletName(WalletWizardComponent, Logger):
    def __init__(self, parent, wizard):
        ...
        # THIS IS THE FILE WE WILL CHANGE
        hbox = QHBoxLayout()
...
class WCWalletType(WalletWizardComponent):
    def __init__(self, parent, wizard):
        ...
        choices.append(ChoiceItem(key='retrieve_vt_seed', label='Retrieve seed phrase via Virtual Token'))
...
class WCConfirmSeed(WalletWizardComponent):
    def __init__(self, parent, wizard):
        ...
        self.seed_widget.validChanged.connect(self.seed_valid_changed)
        self.layout().addWidget(self.seed_widget)

        self.save_seed_virtual_token_button = QPushButton('Save Seed Phrase with Virtual Token')
        self.save_seed_virtual_token_button.setEnabled(False)
        self.save_seed_virtual_token_button.clicked.connect(self.on_save_seed_virtual_token_clicked)
        button_row = QHBoxLayout()
        button_row.addWidget(self.save_seed_virtual_token_button)
    ...
    def on_save_seed_virtual_token_clicked(self):
        if not self.valid:
            return
        self.wizard_data['save_seed_virtual_token'] = True
        view = self.wizard.submit(self.wizard_data.copy())
        self.wizard.load_next_component(view.view, view.wizard_data, view.params)
        self.wizard.set_default_focus()

    def seed_valid_changed(self, valid):
        self.valid = valid
        self.save_seed_virtual_token_button.setEnabled(valid)


class WCSaveSeedVirtualToken(WalletWizardComponent):
    def __init__(self, parent, wizard):
        WalletWizardComponent.__init__(
            self,
            parent,
            wizard,
            title='Save Seed Phrase with Virtual Token',
        )

        self.layout().addWidget(
            WWLabel(
                "The virtual token system allows users to securely save their seed phrase for their new wallet.\n"
                "Users will select a virtual token (something unique to them hopefully) and it will be encrypted and distributed\n"
                "between a choice of usb, Google Drive, and a secure database. All components of the virtual token system\n"
                "are stored off of the device running the Electrum wallet, so if you lose your physical copy of your seed phrase\n"
                "(don't do that) you can still retrieve the seed phrase so you can still use it."
            )
        )

        # Buttons
        self.enroll_button = QPushButton('Enroll')
        self.enroll_button.clicked.connect(self.on_enroll_clicked)

        self.done_button = QPushButton('Cancel')
        self.done_button.clicked.connect(self.on_cancel_clicked)
    ...
    def on_enroll_clicked(self):
        # Stub: advance wizard or open enrollment flow later
        self.wizard.requestNext.emit()

    def on_cancel_clicked(self):
        self.wizard.requestPrev.emit()

class WCVirtualTokenEnrollment(WalletWizardComponent):
    """
    Wizard component for encrypting and enrolling a file with virtual token.
    Follows the WalletWizardComponent pattern from wallet.py while implementing
    the enrollment functionality from enroll_page.py.
    """
    ...
    def __init__(self, parent, wizard):
        WalletWizardComponent.__init__(
            ...
            parent,
            wizard,
            title='Enroll With Virtual Token',
        )
        ...
        self.user_id_edit.setPlaceholderText('Enter your user ID')
        form_layout.addRow('User ID:', self.user_id_edit)
        
        # File to enroll selection
        file_layout = QHBoxLayout()
        self.file_path_edit = QLineEdit()
        self.file_path_edit.setPlaceholderText('Select a file to enroll')
        self.file_path_edit.setReadOnly(True)
        self.browse_file_button = QPushButton('Browse...')
        self.browse_file_button.clicked.connect(self.browse_file)
        file_layout.addWidget(self.file_path_edit)
        file_layout.addWidget(self.browse_file_button)
        form_layout.addRow('File to Enroll:', file_layout)
        
        # File description input
        self.description_edit = QLineEdit()
        self.description_edit.setPlaceholderText('Enter a description for the file')
        form_layout.addRow('File Description:', self.description_edit)
        
        # Key storage path selection
        self.keys_storage_combo = QComboBox()
        self.keys_storage_combo.addItem("— Select Keys Storage —", None)
        self.keys_storage_combo.addItem("USB", "usb")
        self.keys_storage_combo.addItem("Google Drive", "google_drive")
        self.keys_storage_combo.currentIndexChanged.connect(self._on_keys_storage_changed)
        form_layout.addRow("Keys Storage Location", self.keys_storage_combo)
        
        # File storage path selection
        self.file_storage_combo = QComboBox()
        self.file_storage_combo.addItem("— Select File Storage —", None)
        self.file_storage_combo.addItem("USB", "usb")
        self.file_storage_combo.addItem("Database", "database")
        self.file_storage_combo.addItem("Google Drive", "google_drive")
        self.file_storage_combo.currentIndexChanged.connect(self._on_file_storage_changed)
        form_layout.addRow("Encrypted File Storage Location", self.file_storage_combo)
        
        # USB drive path selection
        usb_layout = QHBoxLayout()
        self.usb_path_edit = QLineEdit()
        self.usb_path_edit.setPlaceholderText('Select USB drive path')
        self.usb_path_edit.setReadOnly(True)
        self.browse_usb_button = QPushButton('Browse...')
        self.browse_usb_button.clicked.connect(self.browse_usb_path)
        usb_layout.addWidget(self.usb_path_edit)
        usb_layout.addWidget(self.browse_usb_button)
        form_layout.addRow('USB Drive Path:', usb_layout)
        ...
        # Encrypt and Enroll button
        self.enroll_button = QPushButton('Encrypt and Enroll')
        self.enroll_button.clicked.connect(self.on_enroll_clicked)
        self.layout().addWidget(self.enroll_button)
    
    def browse_file(self):
        """Open file dialog to select file to enroll"""
        file_path, _ = QFileDialog.getOpenFileName(
            self, 'Select File to Enroll',
            '', 'All Files (*)'
        )
        if file_path:
            self.file_path_edit.setText(file_path)
    
    def _on_keys_storage_changed(self):
        """Handle keys storage dropdown change."""
        keys_storage = self.keys_storage_combo.currentData()
        file_storage = self.file_storage_combo.currentData()
        
        if keys_storage and file_storage and keys_storage == file_storage:
            QMessageBox.warning(
                self, 
                "Invalid Storage Selection",
                "Keys and encrypted file must be stored in different locations.\nPlease choose a different storage location."
            )
            self.keys_storage_combo.setCurrentIndex(0)  

    def _on_file_storage_changed(self):
        """Handle file storage dropdown change."""
        keys_storage = self.keys_storage_combo.currentData()
        file_storage = self.file_storage_combo.currentData()
        ...
        if keys_storage and file_storage and keys_storage == file_storage:
            ...
            self.file_storage_combo.setCurrentIndex(0) 
    
    def browse_usb_path(self):
        """Open directory dialog to select USB drive path"""
        ...
        if dir_path:
            self.usb_path_edit.setText(dir_path)
    
    def on_enroll_clicked(self):
        """Handle the Encrypt and Enroll button click"""
        ...
        if not user_id:
            QMessageBox.warning(self, "Missing User ID", "Please enter your user ID.")
            return
        ...
        if not keys_storage:
            QMessageBox.warning(self, "Missing Storage", "Please select a keys storage location.")
            return
        if not file_storage:
            QMessageBox.warning(self, "Missing Storage", "Please select an encrypted file storage location.")
            return
        if keys_storage == file_storage:
            QMessageBox.warning(self, "Invalid Storage", "Keys and encrypted file must be stored in different locations.")
            return
        
        usb_path = self.usb_path_edit.text().strip() or None
        if not usb_path:
            QMessageBox.warning(self, "Missing USB Path", "Please specify a USB storage path for file_info.json.")
            return
        
        storage_pw = self.password_edit.text().strip() or None
        if not storage_pw:
            QMessageBox.warning(self, "Missing Password", "Please enter a storage password.")
            return

        description = self.description_edit.text().strip()

        # Disable UI while enrolling
        ...
        self._worker.finished.connect(self._on_enrollment_finished)
        self._worker.failed.connect(self._on_enrollment_failed)
    ...
    def _set_ui_enabled(self, enabled: bool):
        ...
        self.browse_file_button.setEnabled(enabled)
        self.description_edit.setEnabled(enabled)
        self.keys_storage_combo.setEnabled(enabled)
        self.file_storage_combo.setEnabled(enabled)
        self.browse_usb_button.setEnabled(enabled)
        self.password_edit.setEnabled(enabled)

    def _on_progress(self, message: str, level: str):
        """Handle progress messages from the worker."""
        # Log to console; a log_widget could be added here if desired
        import logging
    ...
    def _on_enrollment_finished(self, result: dict):
        """Called on the GUI thread when enrollment completes."""
        ...
        if result.get("success"):
            ...
            QMessageBox.information(
                ...
                "File enrolled successfully in {:.2f}s.".format(duration),
            )
            self._enrollment_result = result
            self.valid = True
            self.apply()
            self.wizard.requestNext.emit()
        else:
            msg = result.get("message", "Unknown error")
            QMessageBox.critical(self, "Enrollment Failed", msg)

    def _on_enrollment_failed(self, error: str):
        """Called on the GUI thread when an unexpected exception occurs."""
        self._set_ui_enabled(True)
        QMessageBox.critical(self, "Enrollment Error", error)
    ...
    def validate(self):
        # Validation happens after successful enrollment via signal
        pass

    def apply(self):
        if getattr(self, "_enrollment_result", None):
            self.wizard_data["virtual_token_enrollment"] = self._enrollment_result

class WCVirtualTokenEncryptSeed(WalletWizardComponent):
    """After VT enrollment: recover ephemeral key, encrypt seed, save to USB."""

    def __init__(self, parent, wizard):
        WalletWizardComponent.__init__(
            self, parent, wizard,
            title='Encrypt Seed Phrase with Virtual Token',
        )

        self.layout().addWidget(WWLabel(
            "The enrollment is complete. Now we will recover the encryption key from your\n"
            "virtual token and use it to encrypt your seed phrase.\n\n"
            "Choose a location to save the encrypted seed file."
        ))

        form = QFormLayout()
        usb_layout = QHBoxLayout()
        self.usb_path_edit = QLineEdit()
        self.usb_path_edit.setPlaceholderText('Select USB / folder to save encrypted seed')
        self.usb_path_edit.setReadOnly(True)
        self.browse_btn = QPushButton('Browse...')
        self.browse_btn.clicked.connect(self._browse_usb)
        usb_layout.addWidget(self.usb_path_edit)
        usb_layout.addWidget(self.browse_btn)
        form.addRow('Save Location:', usb_layout)
        self.layout().addLayout(form)

        self.encrypt_btn = QPushButton('Encrypt && Save Seed')
    ...
    def on_ready(self):
        self.valid = False
        # Pre-fill USB path from enrollment if available
        enrollment = self.wizard_data.get('virtual_token_enrollment', {})
        usb = enrollment.get('usb_path', '')
        if usb:
            self.usb_path_edit.setText(usb)

    def _browse_usb(self):
        dir_path = QFileDialog.getExistingDirectory(self, 'Select Save Location', '')
        if dir_path:
            self.usb_path_edit.setText(dir_path)

    def _on_encrypt_clicked(self):
        save_dir = self.usb_path_edit.text().strip()
        if not save_dir:
            QMessageBox.warning(self, 'Missing Path', 'Please select a save location.')
            return

        seed = self.wizard_data.get('seed', '')
        if not seed:
            QMessageBox.critical(self, 'Error', 'No seed phrase found.')
            return

        enrollment = self.wizard_data.get('virtual_token_enrollment', {})
        user_id = enrollment.get('file_info', {}).get('user_id', '') or ''
        storage_pw = enrollment.get('storage_password', '')
        keys_storage = enrollment.get('keys_storage', 'usb')
        file_storage = enrollment.get('file_storage', 'database')
        usb_path = enrollment.get('usb_path', save_dir)

        # Build a minimal file_info dict from enrollment result
        file_info = enrollment.get('file_info', {})

        self.encrypt_btn.setEnabled(False)
        self.status_label.setText('Recovering encryption key from virtual token...')
        QApplication.processEvents()
...
class WCRetrieveSeedVirtualToken(WalletWizardComponent):
    """Wallet-chooser page: recover seed phrase via Virtual Token."""

    def __init__(self, parent, wizard):
        WalletWizardComponent.__init__(
            self, parent, wizard,
            title='Retrieve Seed Phrase via Virtual Token',
        )

        self.layout().addWidget(WWLabel(
            "Recover your seed phrase using your Virtual Token.\n"
            "Enter your User ID and password, then select the file_info.json\n"
            "that was saved during enrollment. The keys and encrypted files\n"
        ...
        self.file_info_edit = QLineEdit()
        self.file_info_edit.setPlaceholderText('Select file_info.json')
        self.file_info_edit.setReadOnly(True)
        fi_btn = QPushButton('Browse...')
        fi_btn.clicked.connect(self._browse_file_info)
        fi_layout.addWidget(self.file_info_edit)
        fi_layout.addWidget(fi_btn)
        form.addRow('File Info:', fi_layout)

        self.layout().addLayout(form)

        self.retrieve_btn = QPushButton('Retrieve Seed Phrase')
        self.retrieve_btn.clicked.connect(self._on_retrieve_clicked)
        self.layout().addWidget(self.retrieve_btn)

        self.result_label = WWLabel('')
        self.layout().addWidget(self.result_label)
        self.layout().addStretch(1)

    def _browse_file_info(self):
        path, _ = QFileDialog.getOpenFileName(
            self, 'Select file_info.json', '',
            'JSON Files (*.json);;All Files (*)'
        )
        if path:
            self.file_info_edit.setText(path)
    ...
    def _on_retrieve_clicked(self):
        ...
        file_info_path = self.file_info_edit.text().strip()

        if not user_id:
            QMessageBox.warning(self, 'Missing', 'Please enter your User ID.'); return
        if not password:
            QMessageBox.warning(self, 'Missing', 'Please enter your password.'); return
        if not file_info_path:
            QMessageBox.warning(self, 'Missing', 'Please select file_info.json.'); return

        self.retrieve_btn.setEnabled(False)
        self.result_label.setText('Loading file info and recovering key...')
        QApplication.processEvents()

        try:
            # Load file_info — contains filename, keys_storage, file_storage, etc.
            with open(file_info_path, 'r', encoding='utf-8') as f:
                file_info = _json.load(f)

            keys_storage = file_info.get('keys_storage', 'usb')
            file_storage = file_info.get('file_storage', 'database')
            ...
            if not os.path.isfile(seed_file_path):
                raise FileNotFoundError(
                    f'Encrypted seed file not found at expected location:\n{seed_file_path}'
                )
            ...
            if len(encrypted_seed) < 16:
                raise ValueError('Encrypted seed file is too short.')
            ...
            self.result_label.setText(
                '<b>Recovered Seed Phrase:</b><br><br>' + seed_phrase
            )
            self.valid = True

        except Exception as e:
            QMessageBox.critical(self, 'Error', str(e))
            self.result_label.setText('Retrieval failed.')
        finally:
